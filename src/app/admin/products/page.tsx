...
export default function AdminProductsPage() {
  ...
  const { data: blacklistedResponse, refetch: refetchBlacklisted } =
    useGetBlacklistedProductsQuery(undefined, { skip: !showBlacklisted });
  const { data: instsResponse, refetch: refetchInsts } =
    useGetInstitutionsQuery();

  const [createProduct, { isLoading: isPublishing }] =
    useCreateProductMutation();
  ...
  const [restoreProduct, { isLoading: isRestoring }] =
    useRestoreProductMutation();
  const [createInstitution, { isLoading: isSavingProvider }] =
    useCreateInstitutionMutation();
  ...
  const products = showBlacklisted
  ...
  const handleBlacklistProduct = async (id: string) => {
    const reason = window.prompt("Enter blacklist reason", "Payment failure");
    if (!reason) {
      toast.error("Blacklist reason is required");
      return;
    }
    try {
      const res = await blacklistProduct({ id, reason }).unwrap();
      if (res.success) {
        toast.success("Product blacklisted successfully");
        refetchProducts();
        refetchBlacklisted();
      }
    } catch (err: any) {
      toast.error(err.data?.message || "Error blacklisting product");
    }
  };
  ...
  const handleRestoreProduct = async (id: string) => {
    try {
      ...
      toast.error(err.data?.message || "Error restoring product");
    }
  };
  ...
  const handleRegisterProvider = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const newlyAdded = refetched?.data?.find(
          (i: any) => i.name === res.data?.name,
        );
        if (newlyAdded) {
  ...
  return (
    <AdminShell>
        ...
        >
          <div>
            ...
            <p style={{ margin: "8px 0 0", fontSize: 13, color: C.textSub }}>
              {showBlacklisted
                ? "Review blacklisted listings, restore eligible products, and manage payment-flagged inventory."
          ...
          >
            <button
              ...
              style={{
                background: showBlacklisted ? C.surface : C.blue,
                color: showBlacklisted ? C.text : "#fff",
            ...
            <button
              ...
              style={{
                background: showBlacklisted ? C.blue : C.surface,
                color: showBlacklisted ? "#fff" : C.text,
                border: `1px solid ${C.border}`,
                borderRadius: 10,
                padding: "10px 18px",
                fontSize: 12.5,
        ...
        {productsLoading ? (
          <div style={{ padding: "60px 0", textAlign: "center" }}>
            <p style={{ color: C.textSub, fontSize: 14 }}>
              Synchronizing catalog listings...
            </p>
          </div>
          ...
          >
            ...
            <p style={{ color: C.textSub, fontSize: 14, fontWeight: 600 }}>
              No products defined inside the ecosystem.
            </p>
            <p style={{ color: C.textMuted, fontSize: 12, marginTop: 4 }}>
              Publish products directly using the launch widget above.
            </p>
          </div>
        ) : (
          <div
              ...
              >
                ...
                >
                  <div>
                    ...
                    <p style={{ margin: 0, fontSize: 12.5, color: C.textSub }}>
                      {p.institutionId?.name || "Institution partner"}
                    </p>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    {p.imageUrl ? (
                      ...
                      <StorefrontRounded
                        sx={{ fontSize: 20, color: C.textSub }}
                      />
                    )}
                  </div>
                </div>
                ...
                >
                  <div>
                    ...
                    >
                      ...
                      >
                        {p.interestRate}%{" "}
                        {p.productType === "Insurance" ? "premium" : "p.a."}
                      </span>
                    </div>
                  </div>
                  <div>
                ...
                >
                  <div>
                    ...
                    >
                      GH₵ {p.minAmount?.toLocaleString()} - GH₵{" "}
                  ...
                  <span
                    style={{
                      fontSize: 10.5,
                      color: p.isBlacklisted ? C.red : C.emerald,
                      fontWeight: 800,
                      display: "inline-flex",
                      alignItems: "center",
                      gap: 4,
                  ...
                  >
                    ...
                    {p.isBlacklisted ? "BLACKLISTED" : "ACTIVE"}
                  </span>
                </div>
                ...
                >
                  ...
                  {!p.isBlacklisted ? (
                    <button
                      onClick={() => handleBlacklistProduct(p._id)}
        ...
        >
          ...
          >
            ...
            {isSuperAdmin && (
              <div>
                ...
                >
                  ...
                  >
                    <AddRounded sx={{ fontSize: 12 }} /> Add Small Lender /
                    Individual
                  </button>
                </div>
                <select
                ...
                >
                  ...
                  {institutions.map((inst: any) => (
                    <option key={inst._id} value={inst._id}>
                      {inst.name} (
                      {inst.type === "Merchant" ? "BNPL Owner" : inst.type})
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div>
              ...
              >
                <option value="Loan">
                  Loan (Small Institutions or Individuals)
                </option>
                <option value="BNPL">
            ...
            {prodType === "BNPL" && (
              ...
              >
                <div>
                  ...
                  >
                    Upload a custom product image or select a high-fidelity
                    preset.
                  </p>
                </div>
                ...
                <label
                  style={{
                    ...
                    justifyContent: "center",
                    height: 120,
                    border: `2px dashed ${imageUrl ? C.emeraldLight + "44" : C.borderStrong}`,
                    borderRadius: 12,
                    cursor: "pointer",
                    ...
                    {[
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setImageUrl(t.img);
                          toast.success(
                            `Preset "${t.label}" loaded successfully!`,
                          );
                        }}
                        style={{
                          padding: "8px 10px",
            ...
            >
              <div>
                <span style={{ fontSize: 12, color: C.textSub }}>
                  Interest Rate (
                  {prodType === "Insurance" ? "monthly fee" : "% p.a."})
                </span>
                <input
                  type="number"
                  step="0.1"
            ...
            <div>
              <span style={{ fontSize: 12, color: C.textSub }}>
                Client Underwriting Requirements
              </span>
              <textarea
          ...
          >
            ...
            <div>
              ...
              <input
                placeholder={
                  provType === "Individual"
                    ? "e.g. Samuel Anim (Lender)"
                    : "e.g. Ebenezer Credit Co."
                }
                value={provName}
                onChange={(e) => {
      ...
      >
        ...
        >
          Are you sure you want to permanently delete the financial product{" "}
